...
def creat_backup_binfile(backup_path):
    ...
    if bin_path_5 == '':
        ...
        backup_tp1_crc = reversal(CRC16_XMODEM(backup_tp1_data).zfill(8))
        backup_tp1 = 'A5A5A5A5' + backup_tp1_len + backup_tp1_crc + drift_4k(backup_tp1_data)
        if (len(backup_tp1) / 2) < 2048:
            looptime = int(2048 - (len(backup_tp1) / 2))
            for i in range(looptime):
                backup_tp1 = backup_tp1 + 'FF'
...
def start():
    ...
    bin_path_1 = filepath + "app.bin"         #不带flag的APP.bin
    ...
    s19_file_path = s19file_path
    intTobin(split_into_array(getS19Data(s19_file_path)),bin_path_1)    # 生成APP.bin
    ...
    final_binfilepath = filepath + 'final.bin'  # bin文件路径
    backup_path = filepath + "backup.bin"       # 备份升级文件路径
