...
def CRC16_CCITT(read):  # 获取CRC-16 XMODEM校验码时实参必须是字符串,返回也是字符串
    ...
    crc_data = crc_data[2:] + crc_data[:2]
...
def creat_backup_binfile(backup_path):
    global s19file_path,bin_path_1,bin_path_2,bin_path_3,bin_path_4,bin_path_5, bin_path_6, bin_path_7, bin_path_8
    backup_app_data = ''
    backup_boot_data = ''
    backup_tcon_data = ''
    backup_key_data = ''
    ...
    backup_icon_data = element_into_str(readbinfile(bin_path_6))
    backup_fpga_data = element_into_str(readbinfile(bin_path_7))
    backup_osd_data = element_into_str(readbinfile(bin_path_8))
    ...
    if os.path.exists(bin_path_5):  # 如果存在TP2则需要填充TP之间的区域
    ...
    elif os.path.exists(bin_path_4):  # 不存在TP2 则只填充TP1
        if backup_tp1_data != '':
            backup_tp1_len = reversal(hex(int(len(backup_tp1_data) / 2)).replace('0x', '').zfill(8))
            backup_tp1_crc = reversal(CRC16_XMODEM(backup_tp1_data).zfill(8))
            backup_tp1 = 'A5A5A5A5' + backup_tp1_len + backup_tp1_crc + drift_4k(backup_tp1_data)
    ...
    # FPGA的备份文件部分
    if backup_fpga_data != '':
        backup_fpga_len = reversal(hex(int(len(backup_fpga_data) / 2)).replace('0x', '').zfill(8))
        backup_fpga_crc = reversal(CRC16_XMODEM(backup_fpga_data).zfill(8))
        backup_fpga = 'A5A5A5A5' + backup_fpga_len + backup_fpga_crc + drift_4k(backup_fpga_data)
        if (len(backup_fpga) / 2) < 3145728:
            looptime = int(3145728 - (len(backup_fpga) / 2))
            for i in range(looptime):
                backup_fpga = backup_fpga + 'FF'
    elif backup_osd_data != '':
        backup_fpga = ''
        looptime = int(3145728)
        for i in range(looptime):
            backup_fpga = backup_fpga + 'FF'
    else:
        backup_fpga = ''
    ...
    if cbox.get() == 'BYD':
        ...
        # TP末尾至ICON起始的填充部分
        if backup_icon_data != '' or backup_fpga_data != '' or backup_osd_data != '':
            looptime = int(196608 - (len(backup_tp) / 2))
            backup_drift = ''
            # 先填充整个TP区域
            for i in range(looptime):
                backup_drift = backup_drift + 'FF'
        ...
        # icon的备份文件部分
        if backup_icon_data != '':
            backup_icon_len = reversal(hex(int(len(backup_icon_data) / 2)).replace('0x', '').zfill(8))
            backup_icon_crc = reversal(CRC16_XMODEM(backup_icon_data).zfill(8))
            backup_icon = 'A5A5A5A5' + backup_icon_len + backup_icon_crc + drift_4k(backup_icon_data)
            if (len(backup_icon) / 2) < 524288:
                looptime = int(524288 - (len(backup_icon) / 2))
                for i in range(looptime):
                    backup_icon = backup_icon + 'FF'

        elif backup_osd_data != '' or backup_fpga_data != '':
            backup_icon = ''
            looptime = int(524288)
            for i in range(looptime):
                backup_icon = backup_icon + 'FF'
        ...
        # OSD的备份文件部分
        if backup_osd_data != '':
            backup_osd_len = reversal(hex(int(len(backup_osd_data) / 2)).replace('0x', '').zfill(8))
            backup_osd_crc = reversal(CRC16_XMODEM(backup_icon_data).zfill(8))
            backup_osd = 'A5A5A5A5' + backup_osd_len + backup_osd_crc + drift_4k(backup_osd_data)
            if (len(backup_osd) / 2) < 2621440:
                looptime = int(2621440 - (len(backup_osd) / 2))
                for i in range(looptime):
                    backup_osd = backup_osd + 'FF'
        else:
            backup_osd = ''

        all_backup = backup_app +  backup_key + backup_tp + backup_drift + backup_icon + backup_fpga + backup_osd

    # 长城项目备份为APP+OSD+TK+TP
    elif cbox.get() == '长城':
        if (len(backup_app)) / 2 < 458752:
            looptime = int(458752 - (len(backup_app) / 2))
            for i in range(looptime):
                backup_app = backup_app + 'FF'
        if backup_osd_data != '':
            backup_osd_len = reversal(hex(int(len(backup_osd_data) / 2)).replace('0x', '').zfill(8))
            backup_osd_crc = reversal(CRC16_XMODEM(backup_osd_data).zfill(8))
            backup_osd = 'A5A5A5A5' + backup_osd_len + backup_osd_crc + drift_4k(backup_osd_data)
            # key 为空则已进行填充。OSD可少填充0x10000的长度
            if (len(backup_osd) / 2) < 327680:
                looptime = int(327680 - (len(backup_osd) / 2))
                for i in range(looptime):
                    backup_osd = backup_osd + 'FF'
        else:
            backup_osd = ''
        all_backup = backup_app + backup_osd +backup_key + backup_tp
...
def start():
    ...
    global s19file_path, flag_s19_path, boot_s19_path, bootmanager_s19_path, bin_path_1, bin_path_2, bin_path_3, bin_path_4, bin_path_5, bin_path_6 , bin_path_7 , bin_path_8
    ...
    if cbox.get() == '长城':
        ...
        bin_path_6 = filepath + "icon.bin"
        bin_path_7 = filepath + "fpga.bin"
        bin_path_8 = filepath + "osd.bin"
        ...
        if (os.path.lexists(bin_path_5)) == False:
            bin_path_5 = ""
        if (os.path.lexists(bin_path_6)) == False:
            bin_path_6 = ""
        if (os.path.lexists(bin_path_7)) == False:
            bin_path_7 = ""
        if (os.path.lexists(bin_path_8)) == False:
            bin_path_8 = ""
        ...
        bin6_data = element_into_str(readbinfile(bin_path_8))  # 获取osd.bin文件的数据
        update_data = bin1_data + bin4_data + bin5_data + bin3_data + bin6_data
        ...
        block5_len = ifnull(hex(int(len(bin6_data) / 2)).replace('0x', '').zfill(8))  # 未进行反转的OSD长度
        ...
        if s19boot_path == "":
            ...
            intTobin(split_into_array(all_data), boot_path)  # creat boot.bin
    ...
    if cbox.get()=='小米':
        ...
        t_component_num = strToasscii(component_num).replace('20','')  # 得到项目名字符串(16进制的ASCII码值)
        t_version_num = strToasscii(version_num).replace('20','')
        ...
        all_data = t_block +  t_component_num + t_version_num + update_data# 反转后的序列4-15内容
