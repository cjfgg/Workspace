...
def split_into_array(s_data):       # 将字符串按两位分割存入数组
    s_data_arr = []
...
def intTobin(arr,binfilepath):    # 该函数形参为字符串数组，binfilepath为要写入的bin文件路径
    binfilepath = open(binfilepath, 'wb')
    for i in arr:
        x = bin(int(i,16)).replace('0b','').zfill(8)     # 字符串形式的二进制
        b = 0b00000000
        for j in x:         # 获取8位01字符，
            if j == '0':
                b = b << 1
...
def element_into_str(arr):  # 将bin文件中的FFFFFFFF后的数据全部提取，再转成字符串
    data_arr = []
    str = ''
...
if __name__ == '__main__':
    ...
    s19file_path = 'C:/Users/艾尔文团长/Desktop/C01/GWM_C01_27inch_Display_APP_Test.s19'
    s19_data = getS19Data(s19file_path)       # 获取S19的数据部分(字符串格式)


    final_binfilepath = 'final.bin'   # bin文件路径
    binfile2_path = 'C:/Users/艾尔文团长/Desktop/C01 file/GW/7901101XKN04A_02.bin'
    binfile3_path = 'C:/Users/艾尔文团长/Desktop/C01 file/GW/7901101XKN04A_03.bin'

    tp_data = element_into_str(readbinfile(binfile2_path))  # 获取TP.bin文件的数据段
    slider_data = element_into_str(readbinfile(binfile3_path))  # 获取slider.bin文件的数据段
    update_data = s19_data + tp_data + slider_data
    headfile_len = '00 00 00 3C'    #
    headfile_len.replace(' ','').zfill(8)
    t_headfile_len = reversal(headfile_len)
    component_num = '7901101XKN04A'             # 字符串类型的零件号 无需反转
    t_component_num = strToasscii(component_num)     # 得到零件号字符串(16进制的ASCII码值)
    version_num = 'SW07.00'        # 以字符串输入无需反转
    ...
    block1_addr = '00014200'        # 未进行反转的block1地址
    t_block1_addr = reversal(block1_addr).zfill(8)
    block1_len = '00040C07'         # 未进行反转的block1长度
    t_block1_len = reversal(block1_len).zfill(8)
    block2_addr = '00060000'        # 未进行反转的block2地址
    t_block2_addr = reversal(block2_addr).zfill(8)
    block2_len = '0000E5A2'         # 未进行反转的block2长度
    t_block2_len = reversal(block2_len).zfill(8)
    block3_addr = ''        # 未进行反转的block3地址
    t_block3_addr = reversal(block3_addr).zfill(8)
    block3_len = ''         # 未进行反转的block3长度
    t_block3_len = reversal(block3_len).zfill(8)
